...
- **`interactive_mode()`**: This method manages the main logic of the program, allowing users to input food items, quantities, and interact with the food database. If a food item is not found, users are prompted to add it to the database. It basically handles everything inside a while loop, therefore until the user doesn't decide to stop adding food, the loop will go on. It makes sense that this method calls a lot of the methods from the previous classes, because this it where the interaction with the user happens. So, in a way, everything comes together here.
```python
